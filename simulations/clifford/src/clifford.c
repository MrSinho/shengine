...
uint64_t SH_ENGINE_EXPORT_FUNCTION attractors_thread(Attractors* p_attractors) {//void* ShEngine::p_engine_extension = NULL
    ...
    {//CLIFFORD ATTRACTOR SAMPLE CODE
        float location[2] = { 0.0f, 0.0f };
        float scale[2] = { 3.0f, 3.0f };
        ...
        p_attractors->p_positions[1] = 1.0f;

        for (uint32_t vertex_idx = 2; vertex_idx < p_attractors->vertex_count; vertex_idx += 2) {
            float* last_pos = &p_attractors->p_positions[vertex_idx - 2];
            float* pos = &p_attractors->p_positions[vertex_idx];
            //if (vertex_idx % 4 == 0) {
            //    memcpy(pos, last_pos, 8);
            //}
            //else {
                pos[0] = last_pos[0] + ((float)sin((double)(a * last_pos[1])) + c * (float)cos((double)(a * last_pos[0]))) * dtime;
                pos[1] = last_pos[1] + ((float)sin((double)(b * last_pos[0])) + d * (float)cos((double)(b * last_pos[1]))) * dtime;
            //}
        }

        for (uint32_t vertex_idx = 0; vertex_idx < p_attractors->vertex_count; vertex_idx++) {
            //p_attractors->p_positions[vertex_idx] += location[vertex_idx % 2];
            p_attractors->p_positions[vertex_idx] *= scale[vertex_idx % 2];
        }
